...
unsigned int ms=0,dd=0,mm=0,i=0;
char buffer[18],buffs[13],buffers[4];
...
void kirim_lcd(void)
{
	...
	{
		lcd_send_cmd(0xc0);
		sprintf(buffer,"lap%dA=%d%d:%d%d:%d%d",(lapA),(mm1/10),(mm1%10),(dd1/10),(dd1%10),(ms1/10),(ms1%10));
		for(i=0;i<100;i++);
		lcd_send_string(buffer);
		lcd_send_cmd(0xd0);
		sprintf(buffers,"Sec");
		lcd_send_string(buffers);

	}
	else if(lapA==totlap){
		lcd_send_cmd(0xc0);
		sprintf(buffer,"mobil A finish   ");
		lcd_send_string(buffer);
		lcd_send_cmd(0xd0);
		sprintf(buffers,"   ");
		lcd_send_string(buffers);
	...
	if(lapB<totlap){
		lcd_send_cmd(0x94);
		sprintf(buffer,"lap%dB=%d%d:%d%d:%d%d",(lapB),(mm2/10),(mm2%10),(dd2/10),(dd2%10),(ms2/10),(ms2%10));
		for(i=0;i<100;i++);
		lcd_send_string(buffer);
		lcd_send_cmd(0xa4);
		sprintf(buffers,"   ");
	...
	if(lapC<totlap){
		lcd_send_cmd(0xd4);
		sprintf(buffer,"lap%dC=%d%d:%d%d:%d%d",(lapC),(mm3/10),(mm3%10),(dd3/10),(dd3%10),(ms3/10),(ms3%10));
		for(i=0;i<100;i++);
		lcd_send_string(buffer);
		lcd_send_cmd(0xe4);
		sprintf(buffers,"Sec");
